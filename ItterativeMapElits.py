...
class EliteGrid:
    ...
    def get_top_child(self, row, col):

        layer = len(self.grids) - 1 

        while layer >= 0 and len(self.grids[layer]) > row and self.grids[layer][row][col] == None:
            layer = layer -1 

        if layer < 0 or  len(self.grids[layer]) <= row:
            return None
        else: 
       
            return self.grids[layer][row][col] 
    ...
    def select(self):
        if(self.pop_size == 0):
            ...
            while(self.get_top_child(row, col) is None):
                row = random.randint(0, self.height - 1)
                col = random.randint(0, self.width - 1)
            return self.get_top_child(row, col)
    
    def _choose_index(self, value):
        increments = 1 / self.height 
        return math.floor(value / increments)
    
    def get_row(self, hint_set, layer = 0):
        solution = hint_set.completed_puzzle.print_grid_small() 
        if solution in self.solutions:
            return self.solutions[solution]
        else:
            row =  self.add_row(layer=layer)
            self.solutions[solution] = row 
            return row
    
    def add_child(self, hint_set, layer = 0):

        if len(self.grids) -1 < layer:
            self.add_layer()

        row = self.get_row(hint_set, layer=layer)
        col = min((hint_set.solver_loops() - 1), self.width - 1)

        self.total_children[row][col] += 1 

    
        curr_top = self.get_top_child(row, col)
        if curr_top is None or hint_set.hint_size() <= curr_top[0]:
            if curr_top == None: 
                self.pop_size += 1 
            self.grids[layer][row][col] = (hint_set.hint_size(), hint_set)
...
# %%
def _add_child(hints, feasible_grid, infeasible, layer):
    """
    ...
    if hints.is_valid():
        ...
        feasible_grid.add_child(hints, layer=layer)
    else:
        fitness = hints.feasibility()
        infeasible.append((fitness, hints)) 

def evolve(puzzle, generations, pop_size, x_rate, mut_rate, add_rate, elits, debug= False, feasible_grid = None):
    if feasible_grid is None:
        feasible_grid = EliteGrid(10) 
        layer = 0 
    else:
        layer = feasible_grid.next_layer()
    infeasible = []
    history = History()
    ...
    for i in range(pop_size):
        ...
        _add_child(hints, feasible_grid, infeasible, layer)
    ...
    for gen in range(generations):
        ...
        for j in range(math.floor(pop_size / 2)):
            ...
            # add children 
            _add_child(child1, feasible_grid, new_infeasible, layer)

          
            _add_child(child2, feasible_grid, new_infeasible, layer)
...
if __name__ == "__main__":
    ...
    pop = evolve(puzzle, 100,10, 0.2, 1, 0.5, 2, debug=True) 
    print(pop[0].get_new_to_layer(0)) 
  

    pop2 = evolve(puzzle, 100,10, 0.2, 1, 0.5, 2, debug=True, feasible_grid=pop[0]) 
    print(pop2[0].get_new_to_layer(1)) 
