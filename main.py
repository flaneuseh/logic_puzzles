...
def get_formatted_unused_grammar(di, cats):
    ...
    if len(empty_is) > 0:
        ...
        return_di["is"] = {
            "logic": "The entity {ent1} in the category {cat1} is connect to the entity {ent2} in the category {cat2}",
            "vars": ["{cat1}", "{cat2}", "{ent1}", "{ent2}"],
            "default_temp": "The {cat1} {ent1} is the {cat2} {ent2}",
            "empty": empty_formatted,
        }
    if len(empty_not) > 0:
        empty_formatted = [{"cat1": e[0], "cat2": e[1]} for e in empty_not]
        return_di["not"] = {
            "logic": "The entity {ent1} in the category {cat1} is not connect to the entity {ent2} in the category {cat2}",
            "vars": ["{cat1}", "{cat2}", "{ent1}", "{ent2}"],
            "default_temp": "The {cat1} {ent1} is not the {cat2} {ent2}",
            "empty": empty_formatted,
        }
    if len(empty_before) > 0:
        empty_formatted = [
            {"cat1": e[0], "cat2": e[1], "num_cat": e[2]} for e in empty_before
        ]
        return_di["before"] = {
            "logic": "The entity {ent1} in the category {cat1} is before/smaller then the {ent2} in the category {cat2}. The amount of which {ent1} is smaller may be specified or unspecified",
            "vars": [
                "{cat1}",
                "{cat2}",
                "{ent1}",
            ...
            "default_temp": "Unspecified version: The {cat1} {ent1} is at least {step} {num_cat} before the {cat2} {ent2}. Specified version: The {cat1} {ent1} is {amount} {num_cat}s before the {cat2} {ent2}",
            "empty": empty_formatted,
        }
    if len(empty_or) > 0:
        empty_formatted = [
            {"cat1": e[0], "cat2": e[1], "is_cat": e[2]} for e in empty_or
        ]
        return_di["or"] = {
            "logic": "Either the entity {ent1} in the category {cat1} or the entity {ent2} in the category {cat2} is connected to the entity {is_ent} in the category {is_cat}, but not both",
            "vars": ["{cat1}", "{cat2}", "{ent1}", "{ent2}", "{is_ent}", "{is_cat}"],
            "default_temp": "Either the {cat1} {ent1} or the {cat2} {ent2} is the {is_cat} {is_ent}",
            "empty": empty_formatted,
        }
    return return_di
...
def get_template():
    ...
    if rule_type == "is":
        ...
        if (
            not cat1 in user_dict
            or not cat2 in user_dict[cat1]
            or not "is" in user_dict[cat1][cat2]
        ):
        ...
        if (
            not cat1 in user_dict
            or not cat2 in user_dict[cat1]
            or not "not" in user_dict[cat1][cat2]
        ):
        ...
        value = {
            "step": 1,
            "untimed": "The {cat1} {ent1} is at least {step} {num_cat} before the {cat2} {ent2}.",
            "timed": " The {cat1} {ent1} is {amount} {num_cat}s before the {cat2} {ent2}",
        }

        if (
            not cat1 in user_dict
            or not cat2 in user_dict[cat1]
            or not num_cat in user_dict[cat1][cat2]
            or not "before" in user_dict[cat1][cat2][num_cat]
        ):
        ...
        if (
            not cat1 in user_dict
            or not cat2 in user_dict[cat1]
            or not is_cat in user_dict[cat1][cat2]
            or not "or" in user_dict[cat1][cat2][is_cat]
        ):
            value = (
...
def update_puzzle():
    ...
    result = Database.update_puzzle(
        username, request_data["key"], request_data["puzzle"]
    )
...
def iter_map_evolve_api(*args):
    ...
    grid = data["grid"]
    # grid = EliteGrid(10)
    elit_grid, infeasible, history = itterative_evolve(
